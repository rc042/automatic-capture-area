...
def screenshot(top_left, bottom_right, next_page_loc, total_page_num):
    ...
    for i in range(total_page_num):
        ...
        # Comment / Uncomment : if you need to clic on the page
        # pyautogui.moveTo(next_page_loc)

        # Comment / Uncomment : if you need to simulate a key
        keyboard = Controller()
        keyboard.press(Key.right)
        keyboard.release(Key.right)

        pyautogui.click(interval=0.2) # 1s pause between actions to be sure the page loads. This can be lowered, but be careful, its taking control of your mouse
        pyautogui.screenshot(region=combined_tuple).save(file_name) # left, top, width, height
        # pyautogui.screenshot(region=(1324, 280, 1335, 1875)).save(file_name) # left, top, width, height
    return images
